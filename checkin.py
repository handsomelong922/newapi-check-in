...
def apply_access_token_auth(client: httpx.Client, headers: dict, raw_token: str):
			...
			try:
				...
				try:
					...
					if 'aliyun_waf_aa' in page_html:
						print(f'[INFO] {account_name}: Detected Aliyun WAF challenge, waiting and reloading...')
				...
				for cookie in cookies:
					...
					if cookie_name in required_cookies or any(key in name_lower for key in ('waf', 'acw', 'aliyun')):
						waf_cookies[cookie_name] = cookie_value

				print(
					f'[INFO] {account_name}: Got {len(all_cookies)} cookies, {len(waf_cookies)} considered WAF-related'
				)

				missing_cookies = [c for c in required_cookies if c not in waf_cookies]

				if missing_cookies:
					print(f'[WARNING] {account_name}: Missing required WAF cookies: {missing_cookies}')
				...
				print(f'[SUCCESS] {account_name}: Successfully prepared browser cookies')
...
def login_with_credentials(
	client, account_name: str, provider_config, username: str, password: str
) -> tuple[bool, str]:
			...
			for request_mode in ('json', 'form'):
				try:
					...
					if request_mode == 'json':
						...
						response = client.post(login_url, json=payload, headers=headers, timeout=30)
					else:
						headers['Content-Type'] = 'application/x-www-form-urlencoded'
						response = client.post(login_url, data=payload, headers=headers, timeout=30)

					# 优先判断 Set-Cookie 场景
					if client.cookies.get('session'):
						print(f'[AUTH] {account_name}: Login successful via {path} ({request_mode})')
						return True, ''
					...
					if response.status_code in (200, 201):
						...
						token = _extract_session_token(result) if isinstance(result, dict) else ''
						if token:
							client.cookies.set('session', token)
							print(f'[AUTH] {account_name}: Login successful, session token obtained')
							return True, ''

						success_flag = bool(result.get('success')) if isinstance(result, dict) else False
						if success_flag and client.cookies.get('session'):
							print(f'[AUTH] {account_name}: Login successful with session cookie')
							return True, ''
		...
		if response.status_code == 200:
			try:
				...
				return {
					'success': False,
					'error': (
						f'Failed to get user info: invalid JSON, content-type={content_type}, body={body_preview}'
					),
				}
...
async def check_in_account(account: AccountConfig, account_index: int, app_config: AppConfig):
	...
	if provider_config.needs_waf_cookies():
		user_cookies = parse_cookies(account.cookies) if account.has_cookies() else {}
		all_cookies = await prepare_cookies(account_name, provider_config, user_cookies)
		if not all_cookies:
			return False, None, None
	elif account.has_cookies() and not account.has_access_token() and not account.has_credentials():
	...
	try:
		...
		if account.has_access_token():
			...
			apply_access_token_auth(client, headers, account.access_token or '')
		elif account.has_credentials():
			# 使用用户名密码登录获取会话
			success, reason = login_with_credentials(
				client, account_name, provider_config, account.username or '', account.password or ''
			)
			if not success:
				print(f'[FAILED] {account_name}: credential login failed - {reason}')
				return False, None, {'success': False, 'error': f'Credential login failed: {reason}'}
		else:
			# cookie 已在上方统一处理
			pass
		...
		if provider_config.needs_waf_cookies() and user_info_before and not user_info_before.get('success'):
			error_text = str(user_info_before.get('error', '')).lower()
			if 'aliyun_waf_aa' in error_text or 'text/html' in error_text:
				print(f'[AUTH] {account_name}: WAF challenge detected, refreshing WAF cookies and retrying...')
				refreshed = await prepare_cookies(account_name, provider_config, {})
				if refreshed:
		...
		if not user_info_before or not user_info_before.get('success'):
			if account.has_access_token():
				print(f'[AUTH] {account_name}: Access token did not work for panel API, fallback to cookie if provided')
				...
				if account.has_credentials():
					print(f'[AUTH] {account_name}: Trying credential login as fallback...')
					...
					if credential_ok:
						...
						print(f'[AUTH] {account_name}: Credential fallback failed - {reason}')
